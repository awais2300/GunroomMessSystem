...
class joto extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $this->load->view('joto/admin');
        } else {
            $this->load->view('joto/login');
        }
    }
    ...
    public function add_users()
    {
        ...
        $this->load->view('joto/create_user', $data);
    }
    ...
    public function login_process()
    {
        if ($this->input->post()) {
            ...
            $query = $this->db->where('username', $username)->where('acct_type', 'joto')->get('security_info')->row_array();
            $hash = $query['password'];
            ...
            if (!empty($query)) {
                if (password_verify($password, $hash)) {
                    ...
                    $this->session->set_flashdata('failure', 'No such user exist. Kindly create New User using joto panel');
                    redirect('Joto');
                }
                //print_r($query); exit; 
            } else {
    ...
    public function update_complaint_process()
    {
        ...
        if (!empty($insert)) {
            ...
            redirect('Joto/complaint');
        } else {
            $this->session->set_flashdata('failure', 'Something went wrong, try again.');
            redirect('Joto/update_complaint');
        }
    }
    public function  update_guest_reservation_process($id = null)
    {
        ...
        if (!empty($insert)) {
            ...
            redirect('Joto/reservation');
        } else {
            $this->session->set_flashdata('failure', 'Something went wrong, try again.');
            redirect('Joto/update_guest_reservation');
        }
    }
    ...
    public function  update_requesting_menu_process($id = null)
    {
        ...
        if (!empty($insert)) {
            $this->session->set_flashdata('success', 'Submitted successfully');
            redirect('Joto/menu_requests');
        } else {
            $this->session->set_flashdata('failure', 'Something went wrong, try again.');
            redirect('Joto/update_menu_requests');
        }
    }
    public function reservation()
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $query = $this->db->set('joto_seen', 'yes')->where('joto_seen', 'no')->update('guest_reservation');
            $this->load->view('joto/reservations', $data);
        }
    }
    public function menu_requests()
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $query = $this->db->set('joto_seen', 'yes')->where('joto_seen', 'no')->update('requesting_menu');
            $this->load->view('joto/menu_requests', $data);
        }
    }
    public function update_guest_reservation($id = null)
    {
        ...
        $this->load->view('joto/update_guest_reservation', $data);
    }
    ...
    {
        $this->load->view('joto/forgot_password');
    }
    ...
    public function get_secret_question()
    {
        $name = $_POST['username'];
        // echo $name; exit;
        $query = $this->db->where('username', $name)->where('username!=', 'joto')->get('security_info')->row_array();
        // print_r($query); exit;
    ...
    public function complaint()
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            // $query = $this->db->set('joto_seen', 'yes')->where('joto_seen', 'no')->update('complaints');
            // if ($query) {
            $this->load->view('joto/select_complaints');
            // }
        }
    }
    ...
    public function show_complaint($type = NULL)
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $query = $this->db->set('joto_seen', 'yes')->where('joto_seen', 'no')->where('account_type',$type)->update('complaints');
            if ($query) {
                $this->load->view('joto/complaint', $data);
            }
        }
    }
    public function update_complaint($complaint_id = null)
    {
        if ($this->session->has_userdata('user_id')) {
            ...
            $this->load->view('joto/update_complaint', $data);
        }
    }
